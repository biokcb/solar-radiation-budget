...
imgDir = "/projects/brkr5980/anom/yearly_imgs/"

fANM   = "annual_anomalies.nc"
fRAW   = "CERES_SYN1.nc"
fPNG   = "land_shallow_topo_2048.tif"
fTime  = "Time_Years.csv"
...
createPseudocolor("skin_av",max=16, min=-16,cmap="YlOrRd3",opType="ColorTable", opacity=1.0)
elevateBy(0,"toa_av")
createIsovolume(lbnd=0, ubnd=125, var="toa_av")
...
for i in range(nFrames):                                #Loop through each frame
  ...
  fImagename = imgDir+"increase_both_view_%04d.png" % i #Set the image name
...
for ts in range(1, 84):              #Loop thru each timestep starting at step 1 NOT 0!
  ...
  attSave.fileName = imgDir+"increase_both_anim_%04d.png" % ts #Set image file name
...
for i in range(nFrames):                                #Loop through each frame
  ...
  p = EvalCubicSpline(t, wghts, vpts)                   #Create cubic spline for camera path for current position 
  SetView3D(p)                                          #Set the view 
  fImagename = imgDir+"increase_both_view_mid_%04d.png" % i #Set the image name
  attSave.fileName = fImagename                         #Save the image name
  SetSaveWindowAttributes(attSave)                      #Set the save attributes  
  SaveWindow()                                          #Save the image

SetActiveTimeSlider(cwDir+fANM)
for ts in range(84, 180):              #Loop thru each timestep starting at step 1 NOT 0!
  title.Delete()
  TimeSliderSetState(ts-1)              #Set the current timestep  
  title = addTitle(tStamp[ts-1])
  
  attSave.fileName = imgDir+"increase_both_anim_mid_%04d.png" % ts #Set image file name
  SetSaveWindowAttributes(attSave)    #Save image file name
  SaveWindow()                        #Save the current image
